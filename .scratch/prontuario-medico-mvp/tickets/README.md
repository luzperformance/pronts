...
**Situação:** proposta para revisão de granularidade e dependências
...
Cada ticket entrega um comportamento observável pela API, com a migração mínima
necessária e testes escritos por TDD. As únicas fronteiras permitidas são:
...
Não fazem parte dos tickets testes isolados de controlador, serviço ou repositório,
duplos de componentes internos, H2, camadas vazias, CRUD genérico ou refatorações
laterais não exigidas pela fatia.

Os tickets ainda não foram enviados a um rastreador. Estes arquivos são a proposta
...
| G-02 | resposta ao baixamento de anexo removido | `410 Gone`, pois a lápide confirma que o recurso existiu | antes de PP-018 |
| G-03 | rastreador do repositório | configurar projeto, rótulos e modelo antes de publicar estes arquivos como itens | antes da publicação externa |

## Topologia de implantação aprovada

- toda a demonstração executada localmente, sem nuvem, domínio público, banco
  externo ou serviço gerenciado;
- imagem Docker de múltiplos estágios executada como usuário sem privilégios de superusuário;
...
- manifestos diretos, sem Helm, Kustomize, operadores, HPA ou GitOps.
...
| [PP-002](PP-002-login-e-contrato-http.md) | Autenticação e contrato HTTP seguro | PP-001 | PP-001 |
...
| [PP-006](PP-006-atualizar-e-alterar-status-do-paciente.md) | Atualizar e alterar estado do paciente | PP-004 | PP-004 |
...
| [PP-020](PP-020-hardening-integrado.md) | Reforço integrado de segurança | PP-019 | PP-019 |
| [PP-021](PP-021-deploy-docker-kubernetes-traefik.md) | Implantação simples com Docker, Kubernetes e Traefik | PP-020 | PP-020 |
...
- quando houver migração, ela sobe em banco vazio e o JPA apenas valida o
  esquema;
- o fluxo REST está coberto com sessão e CSRF quando aplicável;
- regras substanciais estão no domínio e, quando útil, cobertas pela fronteira de
  domínio;
- erros seguem Problem Details e carregam identificador de correlação;
...
- o conjunto relevante e o processo de construção completo permanecem verdes;
- a diferença não introduz itens explicitamente fora do escopo.
